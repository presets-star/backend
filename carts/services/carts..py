...
class CartItemService:
    ...
    def __init__(
            self,
            cart: Cart,
            validated_data: dict[str, Union[Preset, int]]
    ) -> None:
        """Инициализация содержимого корзины."""
        self.cart = cart
        self.validated_data = validated_data
        self.preset = validated_data['preset']
        self.price = self.preset.price
        self.preset_quantity = validated_data['preset'].quantity

    # region ----------------- METHODS CART ITEM SERVICE ----------------------------
    def _is_product_in_cart_items(self) -> None:
        """Есть ли этот продукт в корзине пользователя."""
        if CartItem.objects.filter(cart=self.cart, preset=self.preset):
    ...
    def _add_total_price_product(self) -> None:
        ...
        self.validated_data['total_price_preset'] = self.price * self.quantity
